import fs from 'fs';
import { getImageFullPath, thumbnails, fullpath } from '../utilities/utility';
...
const isImageExists = (
  ...
  const imageDirectoryThumbnail = getImageFullPath(
    thumbnails,
  ...
  try {
    if (fs.existsSync(imageDirectoryThumbnail)) {
      ...
      console.log(
        `Cached image does not exists for 
          image ${filename}. Converting image from original`
      );
